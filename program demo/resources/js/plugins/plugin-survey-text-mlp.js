var jsPsychSurveyTextMLP = (function (jspsych) {
  'use strict';
  ...
  const info = {
      ...
      parameters: {
          ...
          // MLP: vertical = response appears below prompt; horizontal = response appears alongside prompt.
          option_layout: {
            type: jspsych.ParameterType.STRING,
          ...
          // MLP: Set to true to limit input to numbers only
          numeric_only: {
            type: jspsych.ParameterType.BOOL,
            pretty_name: 'Numeric-only input',
            default:  false,
        },
        // MLP: Max input number for numeric-only input (min is assumed to be 0)
          max_number: {
            type: jspsych.ParameterType.INT,
  ...
  SurveyTextMLPPlugin.info = info;

  return SurveyTextMLPPlugin;
