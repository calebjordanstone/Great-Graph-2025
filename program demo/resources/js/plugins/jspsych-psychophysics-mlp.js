 ...
 **/
 ...
 //**modified by MLP and then modified by JC (May 2024). JC modification include drawing a hexagon, diamond and square stimuli**//

 /* global jsPsych, math, numeric */

 var jsPsychPsychophysicsMLP = (function (jspsych) {
  "use strict";
  ...
  const info = {
    ...
    parameters: {
      ...
      // ADDED BY MLP: check_focus_interval
      check_focus_interval: {
        type: jspsych.ParameterType.INT,
        pretty_name: 'Focus check interval',
        default: 0,
        description: 'If set to a positive value, repeatedly returns focus to the named div after this interval (in milliseconds).'
      },
      correct_orientation: {
        type: jspsych.ParameterType.INT,
        pretty_name: 'Record line orientation',
        default: null,
        description: 'Should show either 45 or 90.'
      },
    }
  };
   ...
   */
  class PsychophysicsPluginMLP {
    constructor(jsPsych) {
      this.jsPsych = jsPsych;
    }
    trial(display_element, trial) {

        // ADDED BY MLP: Regularly returns focus to jsPsych display element (if you don't do this and are not using a full-screen display element, then if people have clicked elsewhere they may think they are unable to respond)
      ...
      //square?
      class square_stimulus extends visual_stimulus {
      constructor(stim) {
          super(stim);

          if (typeof this.size === 'undefined') {
              alert('You must specify the size of the square.');
          }

          if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
              alert('You must specify either the line_color or fill_color property.');
          }
      }
      ...
      show() {
          ...
          const diagonalLength = Math.sqrt(2) * this.size; // Diagonal of the square (same as the diameter of the inscribed diamond)
          const sideLength = diagonalLength / Math.sqrt(2); // Side length of the square

          // Define the four points of the square based on the center and side length
          // const halfSide = sideLength / 2;
          const halfSide = (sideLength / 2)*0.8; //this is based on Gaspelin (2016) and Pearson (2020) to produce the same size square and diamond

          const topLeft = [this.currentX - halfSide, this.currentY - halfSide];
          const topRight = [this.currentX + halfSide, this.currentY - halfSide];
          const bottomRight = [this.currentX + halfSide, this.currentY + halfSide];
          const bottomLeft = [this.currentX - halfSide, this.currentY + halfSide];
          ...
          // Fill the square if fill_color is specified
          if (typeof this.fill_color !== 'undefined') {
              ctx.fillStyle = this.fill_color;
              ctx.fill();
          }

          // Draw the outline if line_color is specified
          if (typeof this.line_color !== 'undefined') {
              ctx.strokeStyle = this.line_color;
              ctx.stroke();
          }
    ...
    //         ctx.miterLimit = this.miterLimit;
    //
    //         // Define the four points of the square based on the center and size
    //         const halfSize = this.size / 2;
    //
    //         const topLeft = [this.currentX - halfSize, this.currentY - halfSize];
    //         const topRight = [this.currentX + halfSize, this.currentY - halfSize];
    //         const bottomRight = [this.currentX + halfSize, this.currentY + halfSize];
    //         const bottomLeft = [this.currentX - halfSize, this.currentY + halfSize];
    //
    //         ctx.beginPath();
    //         ctx.moveTo(...topLeft);
    //         ctx.lineTo(...topRight);
    ...
    // }
    //draw hexagon
    class hexagon_stimulus extends visual_stimulus {
    constructor(stim) {
        super(stim);

        if (typeof this.size === 'undefined') {
            alert('You must specify the size of the hexagon.');
        }

        if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
            alert('You must specify either the line_color or fill_color property for the hexagon.');
        }
    }
    ...
    show() {
          ...
          // Define the six points of the hexagon based on the center and size
          const size = this.size / 2;
          const angleStep = (Math.PI * 2) / 6; // Angle between each corner of the hexagon
          const points = [];

          for (let i = 0; i < 6; i++) {
              const angle = angleStep * i;
              const x = this.currentX + size * Math.cos(angle);
              const y = this.currentY + size * Math.sin(angle);
              points.push([x, y]);
          }
          ...
          // Fill the hexagon if fill_color is specified
          if (typeof this.fill_color !== 'undefined') {
              ctx.fillStyle = this.fill_color;
              ctx.fill();
          }
    ...
    class diamond_stimulus extends visual_stimulus {
        constructor(stim) {
            super(stim);

            if (typeof this.size === 'undefined') {
                alert('You have to specify the size of the diamond.');
            }

            if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
                alert('You have to specify either the line_color or fill_color property.');
            }
        }
        ...
        show() {
            ...
            // Define the points of the diamond based on the center and size
            const halfSize = this.size / 2;

            const topPoint = [this.currentX, this.currentY - halfSize];
            const rightPoint = [this.currentX + halfSize, this.currentY];
            const bottomPoint = [this.currentX, this.currentY + halfSize];
            const leftPoint = [this.currentX - halfSize, this.currentY];
            ...
            // Fill the diamond if fill_color is specified
            if (typeof this.fill_color !== 'undefined') {
                ctx.fillStyle = this.fill_color;
                ctx.fill();
            }
      ...
      class star_stimulus extends visual_stimulus {
          constructor(stim) {
              super(stim);

              if (typeof this.size === 'undefined') {
                  alert('You have to specify the size of the star.');
              }
          ...
          show() {
              ...
              // Calculate the height of the triangles based on the equilateral triangle properties
              const height = (Math.sqrt(3) / 2) * this.size;

              // Define a vertical offset to avoid overlapping of the triangles
              const verticalOffset = height / 3;

              // Define points for the upward pointing triangle
              const topPointUp = [this.currentX, this.currentY - verticalOffset];
              const leftPointUp = [this.currentX - this.size / 2, this.currentY + height / 2 - verticalOffset];
              const rightPointUp = [this.currentX + this.size / 2, this.currentY + height / 2 - verticalOffset];

              // Define points for the downward pointing triangle
              const topPointDown = [this.currentX, this.currentY + verticalOffset];
              const leftPointDown = [this.currentX - this.size / 2, this.currentY - height / 2 + verticalOffset];
              const rightPointDown = [this.currentX + this.size / 2, this.currentY - height / 2 + verticalOffset];

              // Draw the upward pointing triangle
              ctx.beginPath();
              ctx.moveTo(...topPointUp);
              ctx.lineTo(...leftPointUp);
              ctx.lineTo(...rightPointUp);
              ctx.closePath();

              // Draw the downward pointing triangle
              ctx.moveTo(...topPointDown);
              ctx.lineTo(...leftPointDown);
              ctx.lineTo(...rightPointDown);
              ctx.closePath();

              // Fill the Star of David if fill_color is specified
              if (typeof this.fill_color !== 'undefined') {
                  ctx.fillStyle = this.fill_color;
                  ctx.fill();
              }
      ...
      //pentagon
      class pentagon_stimulus extends visual_stimulus {
        constructor(stim) {
        super(stim);

        if (typeof this.size === 'undefined') {
            alert('You must specify the size of the pentagon.');
        }

        if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
            alert('You must specify either the line_color or fill_color property for the pentagon.');
        }
      }
    ...
    show() {
          ...
          // Define the five points of the pentagon based on the center and size
          const size = this.size / 2;
          const angleStep = (Math.PI * 2) / 5; // Angle between each corner of the pentagon
          const points = [];

          for (let i = 0; i < 5; i++) {
              const angle = angleStep * i - Math.PI / 2; // Adjust starting angle to have one point at the top
              const x = this.currentX + size * Math.cos(angle);
              const y = this.currentY + size * Math.sin(angle);
              points.push([x, y]);
          }
          ...
          // Fill the pentagon if fill_color is specified
          if (typeof this.fill_color !== 'undefined') {
              ctx.fillStyle = this.fill_color;
              ctx.fill();
          }
      ...
      //add heptagon
      class heptagon_stimulus extends visual_stimulus {
          constructor(stim) {
              super(stim);

              if (typeof this.size === 'undefined') {
                  alert('You must specify the size of the heptagon.');
              }

              if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
                  alert('You must specify either the line_color or fill_color property for the heptagon.');
              }
          }
          ...
          show() {
              ...
              // Define the seven points of the heptagon based on the center and size
              const size = this.size / 2;
              const angleStep = (Math.PI * 2) / 7; // Angle between each corner of the heptagon
              const points = [];
              ...
              // Fill the heptagon if fill_color is specified
              if (typeof this.fill_color !== 'undefined') {
                  ctx.fillStyle = this.fill_color;
                  ctx.fill();
              }
      ...
      //decagon (ten sided polygon) that will look most like a circle?
      class decagon_stimulus extends visual_stimulus {
          constructor(stim) {
              super(stim);

              if (typeof this.size === 'undefined') {
                  alert('You must specify the size of the decagon.');
              }

              if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') {
                  alert('You must specify either the line_color or fill_color property for the decagon.');
              }
          }
          ...
          show() {
              ...
              // Define the ten points of the decagon based on the center and size
              const size = this.size / 2;
              const angleStep = (Math.PI * 2) / 10; // Angle between each corner of the decagon
              const points = [];
              ...
              // Fill the decagon if fill_color is specified
              if (typeof this.fill_color !== 'undefined') {
                  ctx.fillStyle = this.fill_color;
                  ctx.fill();
              }
      ...
      class triangle_stimulus extends visual_stimulus {
          constructor(stim) {
              super(stim);

              if (typeof this.base === 'undefined' || typeof this.height === 'undefined') {
                  alert('You must specify the base and height for the triangle.');
              }
          ...
          show() {
              ...
              //attempt to shift the position of the triangle so it's not so different to the location of other shapes  -offset = 5 works ok?
              const offset = 0;
              // Define the three points of the triangle
              const halfHeight = this.height / 3; // change from /2 so the Y coordinate is centered
              const topPoint = [this.currentX, this.currentY - (1.8 * halfHeight) + offset];
              const leftPoint = [this.currentX - this.base / 2, this.currentY + halfHeight+ offset];
              const rightPoint = [this.currentX + this.base / 2, this.currentY + halfHeight+ offset];
              // //
//            const bottomPoint = [this.currentX, this.currentY + halfSize];
              ...
              // Fill the triangle if fill_color is specified
              if (typeof this.fill_color !== 'undefined') {
                  ctx.fillStyle = this.fill_color;
                  ctx.fill();
              }
      ...
      class circle_stimulus extends visual_stimulus {
        constructor(stim){
          ...
          if (typeof this.line_color === 'undefined' && typeof this.fill_color === 'undefined') alert('You have to specify either of line_color or fill_color for the circle.');
        }
      ...
      const set_instance = {
        ...
        line: line_stimulus,
        rect: rect_stimulus,
        triangle: triangle_stimulus,
        circle: circle_stimulus,
        diamond: diamond_stimulus,
        star: star_stimulus,
        hexagon: hexagon_stimulus,
        pentagon: pentagon_stimulus,
        heptagon: heptagon_stimulus,
        decagon: decagon_stimulus,
        square: square_stimulus,
        text: text_stimulus,
  ...
  PsychophysicsPluginMLP.info = info;

  return PsychophysicsPluginMLP;
})(jsPsychModule);
